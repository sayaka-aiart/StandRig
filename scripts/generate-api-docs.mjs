...
const playbackSnapshot={type:'object',required:['version','sessionId','revision','modelVersion','playing','values','connectedOutputs','outputAcknowledged'],properties:{version:{const:1},sessionId:{type:'string',format:'uuid',description:'Changes on service restart; compare together with modelVersion.'},revision:{type:'integer'},modelVersion:{type:'integer'},playing:{type:'boolean'},physicsEpoch:{type:'integer',minimum:0},motion:{type:'object',properties:{loaded:{type:'boolean'},name:{type:['string','null']},duration:{type:'number'},time:{type:'number'},running:{type:'boolean'},active:{type:'boolean'},ended:{type:'boolean'},speed:{type:'number',minimum:0.1,maximum:4},loop:{type:'boolean'},parameterIds:{type:'array',items:{type:'string'}}}},demo:{type:'object',properties:{active:{type:'boolean'},mode:{type:'string',enum:['showcase-active','mouse-expression']},parameterIds:{type:'array',items:{type:'string'}}}},values:numericValues,lastSource:{type:['string','null']},connectedOutputs:{type:'integer',minimum:0},outputAcknowledged:{const:false},parameters:{type:'array',items:object}}};
