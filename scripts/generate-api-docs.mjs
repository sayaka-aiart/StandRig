...
function endpoint(path,method,summary,bodySchema,parameters) {
  paths[path]??={};
  paths[path][method]={summary,operationId:method+'_'+path.replace(/[^a-zA-Z0-9]+/g,'_'),responses:{200:jsonResponse,400:errorResponse,404:errorResponse,405:errorResponse,409:errorResponse,422:errorResponse,500:errorResponse},...(parameters?{parameters}:{}),...(bodySchema?{requestBody:{required:true,content:{'application/json':{schema:bodySchema}}}}:{})};
}
