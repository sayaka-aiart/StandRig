...
const service = await createLocalService({ dataDir: path.resolve(option('--data-dir') ?? process.env.STANDRIG_DATA_DIR ?? path.join(root, 'workspace')), port, bridgeSessionFile: option('--bridge-session-file') ?? process.env.STANDRIG_BRIDGE_SESSION_FILE, allowLegacyWrites: process.argv.includes('--allow-legacy-writes') });
