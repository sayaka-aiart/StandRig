...
const service = await createLocalService({ dataDir: path.resolve(option('--data-dir') ?? process.env.STANDRIG_DATA_DIR ?? path.join(root, 'workspace')), port, allowLegacyWrites: process.argv.includes('--allow-legacy-writes') });
