...
Brush numeric constraints are generated from property metadata. Tests inspect integer/minimum/maximum/exclusiveMinimum JSON Schema, compare boundary values with core validation and reject invalid dimensions/effect ranges. The real stdio MCP test rejects iterations 0, 51 and 1.5 before sending any HTTP transaction request. No rendering algorithm changed in this follow-up; previous synthetic visual evidence remains limited as documented above. Separate runtime compatibility versioning remains outstanding. Production-character E2E is complete, as confirmed by the project owner on 2026-09-10; it was not rerun during this selector/schema change.

The earlier production-E2E limitation statements record the scope of their individual implementation checks. The project owner subsequently confirmed that production-character E2E itself has been completed.

## Native motion playback — 2026-09-10

Added native StandRig motion v1, pure sampling/transport, HTTP/MCP control and browser controls. Automated coverage includes all four segment kinds, invalid format/keys/ranges/unknown fields, input cloning, adapter issue rejection, deterministic pause/seek/speed/loop/end/replay, channel ownership, demo exclusion, no rig writes, timer cleanup, strict MCP rejection before HTTP, and embedded replay with an already-running render loop. The embedded transport test uses a null drawing context; actual drawing is checked separately in the browser.

Local Windows / Node 24.14.0 build and 16 smoke checks passed. The Node suite now contains 47 passing tests (including the seven motion tests). An isolated browser sample verified file selection, play/pause/stop, looping, keyboard speed and seek controls, and the independent player. The owner-approved private character copy (140 parts, 149 assets) passed five motion-pose numeric QA checks; browser output visibly changed face/body direction, mouth and blink. Stop restored all starting parameters. One private ParamSleeve definition used defaultValue/name; only the test copy mapped these fields to default/label. Read-only PSD parsing produced 140 parts and 118 image assets. Original rig and PSD hashes remained unchanged; private models, PSDs and captures are excluded from distribution.

This is a focused playback regression on the existing character, not a replacement for its already-completed production-character E2E or a new Model Freeze review. Live2D motion3 import and deterministic physics reconstruction on seek remain unsupported. See [MOTION.md](MOTION.md).
